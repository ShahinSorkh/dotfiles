...
myKeysX = brightnessKeys ++ volumeKeys ++ miscKeys
  where
    brightnessKeys = [ ((0, xF86XK_MonBrightnessDown), spawn "xbacklight -dec 10%")
    ...
    volumeKeys = [ ((0, xF86XK_AudioRaiseVolume), spawn "pactl set-sink-volume @DEFAULT_SINK@ +10%")
       , ((0, xF86XK_AudioLowerVolume), spawn "pactl set-sink-volume @DEFAULT_SINK@ -10%")
       , ((0, xF86XK_AudioMute), spawn "pactl set-sink-mute @DEFAULT_SINK@ toggle")
       ]
    miscKeys = [ ((0, xF86XK_PowerDown), spawn "sudo pm-suspend")
        , ((mod1Mask .|. mod4Mask, xK_space), spawn "$HOME/.bin/xmobar/cycle-kbd-layout.sh") -- super+alt+space
        ]
